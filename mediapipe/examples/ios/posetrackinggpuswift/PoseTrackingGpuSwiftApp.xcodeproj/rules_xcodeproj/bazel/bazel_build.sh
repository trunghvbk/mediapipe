...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@rules_xcodeproj_generated//generator/mediapipe/examples/ios/posetrackinggpuswift/xcodeproj:xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/mediapipe/examples/ios/posetrackinggpuswift/xcodeproj/xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/mediapipe/examples/ios/posetrackinggpuswift/xcodeproj/xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/mediapipe/examples/ios/posetrackinggpuswift/xcodeproj/xcodeproj_target_ids")
